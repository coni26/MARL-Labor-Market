...
for ep in range(100000):
    ...
    while not done:
        ...
        state_tensor = tf.convert_to_tensor([salary, alpha])
        ...
        state_history.append([salary, alpha])
        state_next_history.append([next_salary, alpha])
        ...
        if frame_count % update_after_actions == 0 and len(action_history) > batch_size:
            ...
            _alpha = np.array(state_next_sample[:, 1])
            next_reward = model_target(state_next_sample).numpy()
            updated_q_values = done_sample * (rewards_sample + gamma * (_alpha * next_reward[:, 0] + (1 - _alpha) * next_reward[:, 1])) + (1 - np.array(done_sample)) * (rewards_sample + gamma * np.max(next_reward, axis = 1))
    ...
    if ep % 2000 == 0:
        ...
        state_tensor = tf.convert_to_tensor(np.array([w_default, [0] * len(w_default)]).T)
        ...
        ax1.legend()
        state_tensor = tf.convert_to_tensor(np.array([w_default, [0.1] * len(w_default)]).T)
        action_probs = model_target(state_tensor, training=False)
        l_deny = action_probs[:, 0]
        l_accept = action_probs[:, 1]
        ...
        ax2.legend()
        state_tensor = tf.convert_to_tensor(np.array([w_default, [0.2] * len(w_default)]).T)
        action_probs = model_target(state_tensor, training=False)
        l_deny = action_probs[:, 0]
        l_accept = action_probs[:, 1]
        ax3.plot(w_default, l_deny, label='Reject')
        ax3.plot(w_default, l_accept, label='Accept')
        ax3.plot(w_default, u(w_default) / (1-gamma))
        ax3.legend()
        plt.show()
